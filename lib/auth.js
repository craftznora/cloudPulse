// Cognito auth client for CloudPulse.
// Talks directly to the Cognito API with fetch, so no SDK dependency.
// Requires a user pool app client (no secret) with the
// ALLOW_USER_PASSWORD_AUTH flow enabled.
//
// Env vars (Amplify env vars in prod, .env.local in dev):
