...
// Falls back to the mock data in lib/data.js when it is not, so the UI
// keeps working locally without AWS.
...
function normalize(item) {
  ...
  return {
    ...
    attachment: item.attachmentKey
      ? { name: item.attachmentKey.split("/").pop(), type: "FILE", size: "" }
      : null,
...
export async function createFeedback({ title, description, category }) {
  if (!apiEnabled) {
